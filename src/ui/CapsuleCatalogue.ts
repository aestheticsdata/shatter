 ...
 * column it is set in, and what the width pass measures against (SHA-254).
 *
 * The grid (`#capsuleEntries` in `components.css`: left 18, three 148 px
 * tracks, 12 px gaps) is centred on its *tiles*, so its third track starts at
 * 338 on the 480 px stage and the stage cuts it at 480. A line that fits the
 * 148 px column and lands in that column loses its last letters, and which
 * column an entry lands in is the roster's order — a capsule added to an
 * earlier tier moves every one after it — so every line is held to the third
 * column's room.
 */
