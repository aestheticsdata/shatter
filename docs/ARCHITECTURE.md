...
63 capsules, every one of them a plain tier lookup in the drop machinery: a tier
buys tickets in a shuffled bag. 61 of the 63 rows are exactly that; DEMAKE and
...
```text
  ...
  +---------------------------------------------------------------+
  | { id, name, color, letter, ticks, tier, timed, blurb }  x 63   |
  +---------------------------------------------------------------+
        |
  ...
  DropBag.draw(exclude)                  tickets = TIER_TICKETS[tier]
        ...
        |                                93 a pass, drawn without replacement, so
...
`haywireKickIn`, `haywireKicking`, and so on for sixty-three capsules and six
combos.
