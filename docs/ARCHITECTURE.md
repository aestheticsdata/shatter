...
61 capsules, every one of them a plain tier lookup in the drop machinery: a tier
buys tickets in a shuffled bag. 59 of the 61 rows are exactly that; DEMAKE and
...
```text
  ...
  +---------------------------------------------------------------+
  | { id, name, color, letter, ticks, tier, timed, blurb }  x 61   |
  +---------------------------------------------------------------+
        |
  ...
  DropBag.draw(exclude)                  tickets = TIER_TICKETS[tier]
        ...
        |                                91 a pass, drawn without replacement, so
...
`haywireKickIn`, `haywireKicking`, and so on for sixty-one capsules and six
combos.
