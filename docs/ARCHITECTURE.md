...
62 capsules, every one of them a plain tier lookup in the drop machinery: a tier
buys tickets in a shuffled bag. 60 of the 62 rows are exactly that; DEMAKE and
...
```text
  ...
  +---------------------------------------------------------------+
  | { id, name, color, letter, ticks, tier, timed, blurb }  x 62   |
  +---------------------------------------------------------------+
        |
  ...
  DropBag.draw(exclude)                  tickets = TIER_TICKETS[tier]
        ...
        |                                92 a pass, drawn without replacement, so
...
`haywireKickIn`, `haywireKicking`, and so on for sixty-two capsules and six
combos.
