...
> - **The themes cannot take the prototype's sky.** They are authored far darker because `check:backgrounds` demands 3:1 for every sprite against an `area` tone, so a bright `glowHi` or `dome` is refused outright. It is not needed: the win is dithering _between the tones already there_, and every HD-only tone here is `mix()` of the theme's own, always toward its base and never up.
> - **Seeded parity is a rule about the generator, not the drawing.** A painter may draw an element differently at the two resolutions — a lattice line is one fine pixel in HD — as long as `random()` is called the same number of times in the same order. That is what keeps every star, dune and stone in the same place on both paths.
> - **Part A never reaches the effects.** It covers the furniture and jumps to the Observer, but the roster's own drawing — threads, dust, washes, specks — is in `CanvasRenderer`, about 27 effects' worth, and a 1 px mark is a 3x3 block beside an HD wall. In scope, and it added a third primitive: `mote` is a fine position at a _fine_ size, the quadrant `pixel` and `spritePixel` leave empty.
> - **A dashed line's weight is thickness times duty, and the fine grid costs it both.** Converting a thread at `pitch / 3` — the obvious arithmetic — leaves it at a third of its ink, which on a dark field is a cue the player has to hunt for. It has to close twice. `finePitch()` does it and `check:pix` refuses a line that does not.
> - **Finer is not the same as thinner.** A dust grain cut from 3x3 to 2x2 keeps four fine pixels of nine, so the count doubles with it; a trickle that only thinned would be the same trickle further away. Taken only where the count is a drawing loop — a designed arrangement, like ENGLISH's three flecks at 120 degrees, keeps its count and gets a finer speck.
> - **The panel's type sizes do not transfer.** "Silkscreen 16/24 px" is this bundle's in-canvas panel in fine pixels; the repo's panel is DOM, measured in CSS pixels, with columns tuned at 6/7/11/12 px and a width check to enforce them. The fonts and the inset tone already match exactly; the 2 px inset bevel is the one part that ships, and it changes both art modes because the panel is DOM.
...
> - **Part B is a version behind, and now has its own spec: `docs/superpowers/specs/2026-09-21-observer-hd-design.md`.** `2026-09-18-observer-43-levels`, `2026-09-19-observer-one-eye-43-placements` and `2026-09-19-bestiary-and-bosses` landed after this bundle; its eye art knows nothing of the bestiary, the bosses grown by `doubled()`, or THE CHART. The three corrections that matter most: the eye is **fourteen sockets** from hw 16 to hw 150, not THE VEIL's one, so every feature is a fraction of the socket with a floor of one fine pixel rather than an absolute offset off `HW`; the sclera is **drawn, not baked**, for the bricks' reason; and **the Chariot is not adopted** — there is no bronze paddle in the game.
...
> **Superseded by `docs/superpowers/specs/2026-09-21-observer-hd-design.md`** (SHA-225), which measures everything below against the Observer that actually ships. The art is the target; the inventory and the absolute offsets are not.
